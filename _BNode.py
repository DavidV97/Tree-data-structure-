...
class _BNode(object):
    ...
    def remove(self, index, ancestors):
        ...
        if self.children:
            """
            find the smallest in the right subtree, exchange
            the value with the current node then delete the
            smallest one, just like the idea in the binary
            search tree. Note: only if len(descendent.
            contents) > minimum, we do this way in order to
            avoid 'grow' operation. Or we will inspect the
            left tree and do it any way all internal nodes
            ...
            """
            fall back to the left child, and exchange with the
            biggest, then delete the biggest anyway.
            """
